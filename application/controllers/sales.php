...
class Sales extends Secure_area
{
	...
	function select_doctor()
	{
		...
		$this->sale_lib->set_referring_doctor($doctor_id);
		$this->_reload();
	}
	...
	function complete()
	{
		...
		$doctor_id=$this->sale_lib->get_referring_doctor();
		...
		if($doctor_id!=-1)
		{
			$info=$this->Supplier->get_info($doctor_id);
			$data['doctor']=$info->first_name.' '.$info->last_name;
		
		}
		
		
			$receipt_code= $this->config->item('receipt_code');
		//SAVE sale to database
		$data['sale_id']=$receipt_code.' '.$this->Sale->save($data['cart'], $customer_id,$employee_id,$doctor_id, $comment,$receipt_code,$data['payments']);
		if ($data['sale_id'] == $receipt_code.' -1')
		{
	...
	function receipt($sale_id)
	{
		...
		$receipt_code= $this->config->item('receipt_code');
		$data['sale_id']=$receipt_code.' '.$sale_id;
		$this->load->view("sales/receipt",$data);
		$this->sale_lib->clear_all();
	...
	function _reload($data=array())
	{
		...
		$data['payment_options']=array(
			...
			$this->lang->line('sales_mutual') => $this->lang->line('sales_mutual'),
			$this->lang->line('sales_nhis') => $this->lang->line('sales_nhis'),
	...
	function suspend()
	{
		...
		//SAVE sale to database
		$data['sale_id']=$this->$receipt_code.' '.$this->Sale_suspended->save($data['cart'], $customer_id,$employee_id,$comment,$receipt_code,$data['payments']);
		if ($data['sale_id'] == $receipt_code.' -1')
		{
			$data['error_message'] = $this->lang->line('sales_transaction_failed');
		}
