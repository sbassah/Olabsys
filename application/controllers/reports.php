...
{	
 ...
 $this->ckeditor->config['width'] = '1080px';
 ...
 $this->ckfinder->SetupCKEditor($this->ckeditor,'../asset/ckfinder/');		
	...
	function summary_sales($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		{
			$tabular_data[] = array($row['sale_date'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['commission']),to_currency($row['profit']));
		}
	...
	function summary_customers($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{ 
			$tabular_data[] = array($row['customer'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['commission']),to_currency($row['profit']));
		}
	...
	function summary_suppliers($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['doctor'],  $row['test'], to_currency($row['subtotal']),to_currency($row['commission']), to_currency($row['total']));
		}
	...
	function summary_items($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array(character_limiter($row['name'], 16), $row['quantity_purchased'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['commission']), to_currency($row['profit']));
		}
	...
	function summary_employees($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['employee'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']), to_currency($row['commission']), to_currency($row['profit']));
		}
...
foreach($report_data['summary'] as $key=>$row)
		{
			...
			$summary_data[] =array($row['receipt_id'],$row['test_type'], $row['employee_name'], $row['customer_name'] ,$row['printing_time']);
			
				foreach($report_data['details'][$key] as $drow)
			{
				$details_data[$key][] = array($drow['content']);
			}
	...
	//	$headers = $model->getDataColumns();
		...
		$this->load->view("reports/tabular_details1",$data);
		...
		function view($supplier_id=-1)
	{
		...
		$this->load->view("reports/form",$data);
	}
	...
	function specific_customer($start_date, $end_date, $customer_id, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			$summary_data[] = array(anchor('sales/edit/'.$row['sale_id'], $this->config->item('receipt_code').' '.$row['sale_id'], array('target' => '_blank')), $row['sale_date'], $row['items_purchased'], $row['employee_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['commission']), to_currency($row['discount_percent']),to_currency($row['profit']), $row['payment_type'], $row['comment']);
			
			foreach($report_data['details'][$key] as $drow)
			{
				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['quantity_purchased'], to_currency($drow['subtotal']), to_currency($drow['total']), to_currency($drow['tax']),to_currency($drow['commission']),to_currency($drow['discount_percent']),to_currency($drow['profit']));
			}
		}
	...
	function specific_employee($start_date, $end_date, $employee_id, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			$summary_data[] = array(anchor('sales/edit/'.$row['sale_id'], $this->config->item('receipt_code').' '.$row['sale_id'], array('target' => '_blank')), $row['sale_date'], $row['items_purchased'], $row['customer_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']),to_currency($row['commission']),to_currency($row['profit']), $row['payment_type'], $row['comment']);
	...
	function detailed_sales($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			$summary_data[] = array(anchor('sales/edit/'.$row['sale_id'], $this->config->item('receipt_code').' '.$row['sale_id'], array('target' => '_blank')), $row['sale_date'], $row['items_purchased'], $row['employee_name'], $row['customer_name'], to_currency($row['subtotal']), to_currency($row['total']), to_currency($row['tax']) ,to_currency( $row['commission']), to_currency($row['profit']), $row['payment_type'], $row['comment']);
			
			foreach($report_data['details'][$key] as $drow)
			{
				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['quantity_purchased'], to_currency($drow['tax']), to_currency($drow['discount_percent']), to_currency($drow['subtotal']), to_currency($drow['total']),  to_currency($drow['commission']), to_currency($drow['profit']));
			}
		}
	...
	function detailed_receivings($start_date, $end_date, $sale_type, $export_excel=0)
	{
		...
		foreach($report_data['summary'] as $key=>$row)
		{
			...
			foreach($report_data['details'][$key] as $drow)
			{
				$details_data[$key][] = array($drow['name'], $drow['category'], $drow['quantity_purchased'], to_currency($drow['total']), 'GHS '. $drow['discount_percent']);
			}
		}
