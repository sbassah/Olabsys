...

{
 ...
 $success = $this->db->insert('template',$json_data);
return   $success;
	
			}
	
	
		// Update Template data into database
public function update_json_in_db($json_data, $temp_id) 

{
 $this->db->set($json_data);
 
 $this->db->where('id', $temp_id);
$success = $this->db->update('template',$json_data);
...
return  $success;
			...
			// Update Template data into database
public function updated_temp_status($temp_id) 

{

 $this->db->where('id', $temp_id);
 return $this->db->update('template', array('deleted' => 2));
