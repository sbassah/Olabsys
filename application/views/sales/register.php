...
<div id="overall_sale">
	...
	{ echo' <div class="clearfix">&nbsp;</div>';
		echo $this->lang->line("sales_doctor").': <b>'.$doctor. '</b><br />';
		echo anchor("sales/remove_doctor",'['.$this->lang->line('common_remove').' '.$this->lang->line('customers_doctor').']');
	}
	else
	{
		echo form_open("sales/select_doctor",array('id'=>'select_doctor_form')); ?>
		<label id="customer_label" for="doctor"><?php echo $this->lang->line('sales_select_doctor'); ?></label>
		<?php echo form_input(array('name'=>'doctor','id'=>'doctor','size'=>'30','value'=>$this->lang->line('sales_start_typing_doctor_name')));?>
		</form>
		<div style="margin-top:5px;text-align:center;">
		<h3 style="margin: 5px 0 5px 0"><?php echo $this->lang->line('common_or'); ?></h3>
		<?php echo anchor("suppliers/view/-1/width:350",
		"<div class='small_button' style='margin:0 auto;'><span>".$this->lang->line('sales_new_doctor')."</span></div>",
		array('class'=>'thickbox none','title'=>$this->lang->line('sales_new_doctor')));
		?>
		</div>
		<div class="clearfix">&nbsp;</div>
		<?php
...
$(document).ready(function()
{
	...
	$('#item,#customer,#doctor').click(function()
    {
    	$(this).attr('value','');
    });
    ...
    $("#doctor").autocomplete('<?php echo site_url("sales/doctor_search"); ?>',
    {
    	minChars:0,
    	delay:10,
    	max:100,
    ...
    $("#doctor").result(function(event, data, formatted)
    {
		$("#select_doctor_form").submit();
    });

    $('#doctor').blur(function()
    {
    	$(this).attr('value',"<?php echo $this->lang->line('sales_start_typing_doctor_name'); ?>");
    });
