...
class Sale_lib
{
	...
	function get_referring_doctor()
	{
		if(!$this->CI->session->userdata('doctor'))
			$this->set_referring_doctor(-1);

		return $this->CI->session->userdata('doctor');
	}

	function set_referring_doctor($doctor_id)
	{
		$this->CI->session->set_userdata('doctor',$doctor_id);
	}
	...
	{
		$this->CI->session->unset_userdata('doctor');
	}
